	...
	if (fp != NULL) {
		...
		while (fgets(script_line, SCRIPT_COMMAND_SIZE, fp) != NULL && ret_val == 0) {
			...
			if (cindex != 0) {
				...
				if (DataManager::GetIntValue("is_gui_mode")) gui_print("command is: '%s' and there is no value\n", command);
			}
			if (strcmp(command, "install") == 0) {
				...
				// Backup
				if (DataManager::GetIntValue("is_gui_mode")) DataManager::SetValue("tw_action_text2", gui_parse_text("{@backing}"));
				tok = strtok(value, " ");
				strcpy(value1, tok);
				tok = strtok(NULL, " ");
				if (tok != NULL) {
				...
				// Restore
				if (DataManager::GetIntValue("is_gui_mode")) DataManager::SetValue("tw_action_text2", gui_parse_text("{@restore}"));
				...
				if (DataManager::GetIntValue("is_gui_mode")) gui_msg(Msg("restoring=Restoring {1}...")(folder_path));
				...
				if (!TWFunc::Path_Exists(folder_path)) {
					if (DataManager::GetIntValue("is_gui_mode")) gui_msg(Msg(msg::kError, "locate_backup_err=Unable to locate backup '{1}'")(folder_path));
					ret_val = 1;
					continue;
				}
				...
				if (strlen(partitions) != 0) {
					...
					if (DataManager::GetIntValue("is_gui_mode")) gui_msg(Msg("set_restore_opt=Setting restore options: '{1}':")(value2));
					line_len = strlen(value2);
					for (i=0; i<line_len; i++) {
						if ((value2[i] == 'S' || value2[i] == 's') && Partition_List.find("/system;") != string::npos) {
							Restore_List += "/system;";
							if (DataManager::GetIntValue("is_gui_mode")) gui_msg("system=System");
						} else if ((value2[i] == 'D' || value2[i] == 'd') && Partition_List.find("/data;") != string::npos) {
							Restore_List += "/data;";
							if (DataManager::GetIntValue("is_gui_mode")) gui_msg("data=Data");
						} else if ((value2[i] == 'C' || value2[i] == 'c') && Partition_List.find("/cache;") != string::npos) {
							Restore_List += "/cache;";
							if (DataManager::GetIntValue("is_gui_mode")) gui_msg("cache=Cache");
						} else if ((value2[i] == 'R' || value2[i] == 'r') && Partition_List.find("/recovery;") != string::npos) {
							Restore_List += "/recovery;";
							if (DataManager::GetIntValue("is_gui_mode")) gui_msg("recovery=Recovery");
						} else if ((value2[i] == 'B' || value2[i] == 'b') && Partition_List.find("/boot;") != string::npos) {
							Restore_List += "/boot;";
							if (DataManager::GetIntValue("is_gui_mode")) gui_msg("boot=Boot");
						} else if ((value2[i] == 'A' || value2[i] == 'a')  && Partition_List.find("/and-sec;") != string::npos) {
							Restore_List += "/and-sec;";
							if (DataManager::GetIntValue("is_gui_mode")) gui_msg("android_secure=Android Secure");
						} else if ((value2[i] == 'E' || value2[i] == 'e')  && Partition_List.find("/sd-ext;") != string::npos) {
							Restore_List += "/sd-ext;";
							if (DataManager::GetIntValue("is_gui_mode")) gui_msg("sdext=SD-EXT");
						} else if (value2[i] == 'M' || value2[i] == 'm') {
							DataManager::SetValue(TW_SKIP_DIGEST_CHECK_VAR, 1);
							if (DataManager::GetIntValue("is_gui_mode")) gui_msg("digest_check_skip=Digest check skip is on");
						}
					}
				...
				if (is_encrypted) {
					if (DataManager::GetIntValue("is_gui_mode")) gui_err("ors_encrypt_restore_err=Unable to use OpenRecoveryScript to restore an encrypted backup.");
					ret_val = 1;
				} else if (!PartitionManager.Run_Restore(folder_path))
					ret_val = 1;
				else
					if (DataManager::GetIntValue("is_gui_mode")) gui_msg("done=Done.");
			} else if (strcmp(command, "remountrw") == 0) {
				ret_val = remountrw();
			} else if (strcmp(command, "mount") == 0) {
				// Mount
				if (DataManager::GetIntValue("is_gui_mode")) DataManager::SetValue("tw_action_text2", gui_parse_text("{@mounting}"));
				if (value[0] != '/') {
					strcpy(mount, "/");
					strcat(mount, value);
				} else
				...
				if (PartitionManager.Mount_By_Path(mount, true))
					if (DataManager::GetIntValue("is_gui_mode")) gui_msg(Msg("mounted=Mounted '{1}'")(mount));
			} else if (strcmp(command, "unmount") == 0 || strcmp(command, "umount") == 0) {
				// Unmount
				if (DataManager::GetIntValue("is_gui_mode")) DataManager::SetValue("tw_action_text2", gui_parse_text("{@unmounting}"));
				if (value[0] != '/') {
					strcpy(mount, "/");
					strcat(mount, value);
				} else
				...
				if (PartitionManager.UnMount_By_Path(mount, true))
					if (DataManager::GetIntValue("is_gui_mode")) gui_msg(Msg("unmounted=Unounted '{1}'")(mount));
				...
				if (len > strlen(value1) + 1) {
					...
					if (DataManager::GetIntValue("is_gui_mode")) gui_msg(Msg("setting=Setting '{1}' to '{2}'")(value1)(val2));
					DataManager::SetValue(value1, val2);
				} else {
					if (DataManager::GetIntValue("is_gui_mode")) gui_msg(Msg("setting_empty=Setting '{1}' to empty")(value1));
					DataManager::SetValue(value1, "");
				}
			} else if (strcmp(command, "mkdir") == 0) {
				// Make directory (recursive)
				if (DataManager::GetIntValue("is_gui_mode")) DataManager::SetValue("tw_action_text2", gui_parse_text("{@making_dir1}"));
				if (DataManager::GetIntValue("is_gui_mode")) gui_msg(Msg("making_dir2=Making directory: '{1}'")(value));
				...
				if (DataManager::GetIntValue("is_gui_mode")) DataManager::SetValue("tw_action_text2", gui_parse_text("{@running_command}"));
				if (cindex != 0) {
				...
				if (DataManager::GetIntValue("is_gui_mode")) gui_print("%s\n", value);
			} else if (strcmp(command, "sideload") == 0) {
				// ADB Sideload
				if (DataManager::GetIntValue("is_gui_mode")) DataManager::SetValue("tw_action_text2", gui_parse_text("{@sideload}"));
				install_cmd = -1;
				...
				if (DataManager::GetIntValue("is_gui_mode")) gui_msg("start_sideload=Starting ADB sideload feature...");
				ret_val = apply_from_adb("/", &sideload_child_pid);
				if (ret_val != 0) {
					if (ret_val == -2)
						if (DataManager::GetIntValue("is_gui_mode")) gui_err("need_new_adb=You need adb 1.0.32 or newer to sideload to this device.");
					ret_val = 1; // failure
				...
				if (*value) {
					...
					if (DataManager::GetIntValue("is_gui_mode")) gui_err("no_pwd=No password provided.");
					ret_val = 1; // failure
				}
			} else {
		...
		if (DataManager::GetIntValue("is_gui_mode")) gui_msg("done_ors=Done processing script file");
	} else {
		if (DataManager::GetIntValue("is_gui_mode")) gui_msg(Msg(msg::kError, "error_opening_strerr=Error opening: '{1}' ({2})")(SCRIPT_FILE_TMP)(strerror(errno)));
		return 1;
	}

	if (install_cmd && DataManager::GetIntValue(TW_HAS_INJECTTWRP) == 1 && DataManager::GetIntValue(TW_INJECT_AFTER_ZIP) == 1) {
		if (DataManager::GetIntValue("is_gui_mode")) gui_msg("injecttwrp=Injecting TWRP into boot image...");
...
int OpenRecoveryScript::Install_Command(string Zip) {
	...
	if (Zip.substr(0, 1) == "@") {
		...
		if (!PartitionManager.Mount_By_Path(Full_Path, true) || !TWFunc::Path_Exists(Full_Path)) {
			...
			if (DataManager::GetIntValue("is_gui_mode")) gui_msg(Msg(msg::kError, "zip_err=Error installing zip file '{1}'")(Zip));
			return 1;
		}
		LOGINFO("Installing mapped zip file '%s'\n", Full_Path.c_str());
		if (DataManager::GetIntValue("is_gui_mode")) gui_msg(Msg("installing_zip=Installing zip file '{1}'")(Zip));
		...
		if (!TWFunc::Path_Exists(Zip)) {
			...
			if (DataManager::GetIntValue("is_gui_mode")) gui_print("Unable to locate zip file '%s'.\n", Zip.c_str());
			ret_val = 1;
		} else
			if (DataManager::GetIntValue("is_gui_mode"))  gui_msg(Msg("installing_zip=Installing zip file '{1}'")(Zip));
	}
...
void OpenRecoveryScript::Run_OpenRecoveryScript() {
...
int OpenRecoveryScript::Run_OpenRecoveryScript_Action() {
	...
	if (TWFunc::Path_Exists(SCRIPT_FILE_TMP)) {
		if (DataManager::GetIntValue("is_gui_mode")) gui_msg("running_recovery_commands=Running Recovery Commands");
	...
	if (OpenRecoveryScript::check_for_script_file()) {
		if (DataManager::GetIntValue("is_gui_mode")) gui_msg("running_ors=Running OpenRecoveryScript");
		if (OpenRecoveryScript::run_script_file() == 0) {
			reboot = 1;
			op_status = 0;
		}
